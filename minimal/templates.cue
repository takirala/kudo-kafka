...
_templates : {
	service : {
		...
		spec: {
			ports: {
				port: _parameters.BROKER_PORT
				name: "server"
			}
			clusterIP: "None"
			selector: {
				app:      "kafka"
				instance: _parameters.NAME
			}
		}
	}
	...
	statefulset: {
		...
		kind:       "StatefulSet"
		metadata: {
			name:      _parameters.FRAMEWORK_NAME
			namespace: _parameters.NAMESPACE
			labels: {
				kafka: _parameters.FRAMEWORK_NAME
				app:   "kafka"
			}
		}
		spec: {
			...
			serviceName: "\(_parameters.NAME)-svc"
			replicas:    _parameters.BROKER_COUNT
			template: {
				metadata labels: {
					app:   "kafka"
					kafka: _parameters.FRAMEWORK_NAME
				...
				spec: {
					...
					containers: {
						...
						resources requests: {
							memory: _parameters.BROKER_MEM
							cpu:    _parameters.BROKER_CPUS
						}
						ports: {
							containerPort: _parameters.BROKER_PORT
							name:          "server"
						}
						command: [
							"sh",
						...
						env: [{
							name:   "\(_param.KAFKA_HEAP_OPTS)" | value
							value : "-Xmx512M -Xms512M"
						}, {
							name:  "\(_param.KAFKA_OPTS)" | value
						...
						volumes: [{
							name: "config"
							configMap name: "\(_parameters.NAME)-serverproperties"
						}]
					}
				}
			}
